...
export function p22DefaultRows(nowMs) {
  ...
  return [
    ...
    { id: "task-failed", kind: "subagent", status: "failed", label: "Gerar deploy", detail: "Credencial de produção expirou.", startedAtMs: startedMs - 200_000, error: { code: "provider_error", message: "Credencial de produção expirou.", retryable: false }, allowedActions: [] },
...
export async function runP22TasksFixture(ctx) {
  ...
  const syncPush = (frame) => evalExpr("window.__obP22.__pendingFrame = " + JSON.stringify(frame) + "; window.__obP22.stream.emit(window.__obP22.__pendingFrame)");
